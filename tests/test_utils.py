...
sample_constants = dict(
    constant_one=dict(value=10.0, units="J kg^-1 m^-2"),
    constant_two=dict(value=1.002, units="J kg^-1"),
    constant_three=dict(value=1.024e6, units="kg^-1"),
)
...
def test_interface_levels():
    ...
    pressure_interface_level[1:-1, :] = (
        pressure_mid_level[1:, :] + pressure_mid_level[:-1, :]
    ) / 2.0
    ...
    interface_values = get_interface_values(
        mid_level_values, surface_value, pressure_mid_level, pressure_interface_level
    )
...
def test_adding_constant():
    ...
    for constant in sample_constants.keys():
        constant_value = get_constant(constant, sample_constants[constant]["units"])
