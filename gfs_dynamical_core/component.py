...
class GFSDynamicalCore(TendencyStepper):
    ...
    _gfs_input_properties = {
        ...
        "air_pressure": {"dims": ["mid_levels", "lat", "lon"], "units": "Pa"},
    ...
    def __init__(
        ...
        bad_diagnostics = set(
            self._tendency_component.diagnostic_properties.keys()
        ).intersection(self._gfs_diagnostic_properties.keys())
        if len(bad_diagnostics) > 0:
            raise GFSError(
                "Cannot use TendencyComponent components that produce {} as diagnostic"
    ...
    def _initialize_model(self, state, timestep):
        ...
        self._spectral_dim = int((self._truncation + 1) * (self._truncation + 2) / 2)
        ...
        np.testing.assert_allclose(latitudes * 180.0 / np.pi, state["latitude"])
        np.testing.assert_allclose(longitudes * 180.0 / np.pi, state["longitude"])
    ...
    def array_call(self, state, timestep, prognostic_tendencies=None):
        ...
        if timestep.total_seconds() != self._time_step.total_seconds():
            raise GFSError("GFSDynamicalCore can only be run with a constant timestep.")
        ...
        outputs["air_pressure_on_interface_levels"][:] = state[
            "air_pressure_on_interface_levels"
        ][::-1, :, :]
        for name in (
            "air_pressure",
        ...
        outputs["air_pressure_on_interface_levels"][:] = outputs[
            "air_pressure_on_interface_levels"
        ][::-1, :, :]
    ...
    def _get_2d_tendencies(self, tendencies, T_shape):
        return self._get_tendencies(
            tendencies, ["surface_air_pressure"], ["lat", "lon"], T_shape[1:]
        )
    ...
    def _get_tracer_tendencies(self, tendencies, T_shape):
        ...
        for i, name in enumerate(self._tracer_packer.tracer_names):
            if name in tendencies:
                ...
                tend = get_numpy_arrays_with_properties(tendencies, property_dict)[name]
                return_array[i, :, :, :] = tend
            else:
                return_array[i, :, :, :] = 0.0
        return return_array
